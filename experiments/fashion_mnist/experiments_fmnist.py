...
EXPORT_DIR = Path("./output_metrics/fmnist_converged")
...
def add_monitors(network, dt):
    # Metrics
    train_loss_monitor = LossMonitor(export_path=EXPORT_DIR / f"fmnist_loss_train_{dt}")
    train_accuracy_monitor = AccuracyMonitor(export_path=EXPORT_DIR / f"fmnist_accuracy_train_{dt}")
    ...
    test_loss_monitor = LossMonitor(export_path=EXPORT_DIR / f"fmnist_loss_test_{dt}")
    test_accuracy_monitor = AccuracyMonitor(export_path=EXPORT_DIR / f"fmnist_accuracy_test_{dt}")
    test_learning_rate_monitor = ValueMonitor(name="Learning rate", decimal=5)
    # Only monitor LIF layers
    test_spike_counts_monitors = {
        l: SpikeCountMonitor(l.name, export_path=EXPORT_DIR / (f"fmnist_spike_count__{dt}" + str(l.name))) for l in
    ...
    test_norm_monitors = {l: WeightsNormMonitor(l.name, export_path=EXPORT_DIR / (f"fmnist_weight_norm__{dt}" + l.name))
