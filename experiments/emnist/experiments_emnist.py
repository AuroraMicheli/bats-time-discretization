...
def parse_arguments():
    parser = argparse.ArgumentParser(description="Train a spiking neural network.")
    parser.add_argument('--dt', type=float, required=True, help="Time step (DT) value to use for discretization.")
    return parser.parse_args()

# Dataset
DATASET_PATH = Path("../../datasets/emnist-balanced.mat")
PLOT_DIR = Path('./scatter_plots/')
EXPORT_DIR = Path("./output_metrics/emnist_converged")
...
def add_monitors(network, dt):
    # Metrics
    train_loss_monitor = LossMonitor(export_path=EXPORT_DIR / f"emnist_loss_train_{dt}")
    train_accuracy_monitor = AccuracyMonitor(export_path=EXPORT_DIR / f"emnist_accuracy_train_{dt}")
    ...
    test_loss_monitor = LossMonitor(export_path=EXPORT_DIR / f"emnist_loss_test_{dt}")
    test_accuracy_monitor = AccuracyMonitor(export_path=EXPORT_DIR / f"emnist_accuracy_test_{dt}")
    test_learning_rate_monitor = ValueMonitor(name="Learning rate", decimal=5)
    # Only monitor LIF layers
    test_spike_counts_monitors = {
        l: SpikeCountMonitor(l.name, export_path=EXPORT_DIR / (f"emnist_spike_count__{dt}" + str(l.name))) for l in
    ...
    test_norm_monitors = {l: WeightsNormMonitor(l.name, export_path=EXPORT_DIR / (f"emnist_weight_norm__{dt}" + l.name))
...
def train_test(network, dataset, dt, np_seed, cp_seed, train_monitors_manager, test_monitors_manager, train_accuracy_monitor, train_loss_monitor, test_accuracy_monitor, test_loss_monitor):
        plot_count = 0
...
def main():
    if cp.cuda.runtime.getDeviceCount() > 0:
        ...
        print(f"Using GPU: {cp.cuda.runtime.getDeviceProperties(device_id)['name']}")
    else:
    ...
    train_test(network, dataset, dt, np_seed, cp_seed, train_monitors_manager, test_monitors_manager, train_accuracy_monitor, train_loss_monitor, test_accuracy_monitor, test_loss_monitor)
